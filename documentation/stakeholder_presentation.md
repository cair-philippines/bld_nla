...
| 2026-03-25 | Net gain columns | Added 4 columns to composite Excel: Net Gain Trans+ and Net Gain GL for each SY. Net change in students at Transitioning+ and Grade Level from BoSY→EoSY. |
| 2026-03-26 | Assessed count fix | Fixed total assessed to use unique students (from dashboard Total Assessed column) instead of inflated sum of all 30 grade-language columns. Relabeled net gain columns as "Assessments" (Option D). Removed rate columns. Restructured Excel to 2 sheets (All Schools + Notes). Added detailed validation sub-tags to Exclusion Reason. Ranked schools changed from 16,900 to 18,170 (count stability check now uses unique counts). |
| 2026-03-26 | 7 segment pairs + dashboard | Extended pipeline to 7 logical segment pairs (4 within-year + 2 YoY + 1 end-to-end). Updated dashboard TIMEPOINT_ORDER to 5 timepoints. Ranked view period dropdown now reads from priority parquet. Dashboard data regenerated (121,463 priority rows). |
